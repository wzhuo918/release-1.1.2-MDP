...
class ReduceTask extends Task {
  ...
  private int numMaps;//系统中启动的所有map的个数，reduce取数据时要保证每个map输出的相应partition都取到了。
  ...
  public void run(JobConf job, final TaskUmbilicalProtocol umbilical)
    ...
    if (!isLocal) {
      ...
      if (!reduceCopier.fetchOutputs()) {//主要就是这个reduceCopier.fetchOutputs干的
  ...
  class ReduceCopier<K, V> implements MRConstants {
     ...
     */
    private Set <MapOutputLocation> copiedMapOutputs = 
      Collections.synchronizedSet(new TreeSet<MapOutputLocation>());
    
    /** 
     * The set of obsolete（废弃的老旧的过时的） map taskids.
     */
    ...
    private final Map<TaskAttemptID, MapOutputLocation> persistentMapLocations = 
    	new ConcurrentHashMap<TaskAttemptID, MapOutputLocation>();
	...
	public boolean equals(Object o){
    	  ...
    	  if(((ReduceTask.ReduceCopier.MapOutputLocation)o).getTaskAttemptId()==null
    			  ||this.taskAttemptId == null)
    		  return false;
    	  if(((ReduceTask.ReduceCopier.MapOutputLocation)o).getTaskAttemptId().toString().equals(taskAttemptId.toString())
    			  && ((ReduceTask.ReduceCopier.MapOutputLocation)o).getOutputLocation().equals(taskOutput)
    			  )
    			  return true;
		return false;
	...
	public int compareTo(Object o) {
		...
		return 1;
		*/
		return ((ReduceTask.ReduceCopier.MapOutputLocation)o).getTaskId().compareTo(getTaskID().getTaskID());
	}
    }
    ...
    private class MapOutputCopier extends Thread {
      ...
      public void run() {
        while (true) {        
          try {
            ...
            readError = false;
            //LOG.info("------------now coping location:"+loc.getOutputLocation());
            try {
      ...
      /** Copies a a map output from a remote host, via HTTP.   add
...
//    	  LOG.info("--------"+"coping" +loc.getOutputLocation()+"copied map out puts "+ copiedMapOutputs.size());
...
//    		  LOG.info("-------copied map out puts"+copiedMapOutputs.iterator().next());
       ...
       // Path filename =
        ...
        String locUrl = loc.getOutputLocation().toString();
        Path filename =
            new Path(String.format(
            		"%s/map_%d_%s.out",
                TaskTracker.OUTPUT, loc.getTaskId().getId(),
                locUrl.substring(locUrl.indexOf("reduce=")+7, locUrl.length())));
        ...
        synchronized (ReduceTask.this) {
          ...
          //LOG.info("--------------successfully copied map out puts from "+loc.getOutputLocation());
          // Note that we successfully copied the map-output
          noteCopiedMapOutput(loc);
        }
        ...
        while (mergeThrowable == null) {
          synchronized(partitions){//这里和获取事件处理那儿有些同步处理，注意！
        	  if(copiedMapOutputs.size() >= numMaps*partitions.size()){
				...
				{//已经分配的partition已经拷贝完成，等待分配新的partition
					try {
						partitions.wait();
					} catch (InterruptedException e) {
...
//          LOG.info("----!!!!!!!!!!!------partitions:"+partitions+", orign parttion:"+getPartition()+"\n"
//        		  +" copiedmapoutputs.size="+copiedMapOutputs.size()
//        		  +" numMaps"+numMaps+" partitions.size="+partitions.size()
//        		  +" finished:"+partitionArrangeFinished);
          ...
          synchronized (scheduledCopies) {
            ...
            while (hostsItr.hasNext()) {//每一个host最多只取一个mapoutputlocation
          ...
          while (numInFlight > 0 && mergeThrowable == null) {
            ...
            if (cr == null) {
              break;//目前还没有已经copy完成了的mapoutputlocation就直接退出这个循环
            }
        ...
        return mergeThrowable == null && copiedMapOutputs.size() == numMaps*partitions.size();
    }
    ...
    private class InMemFSMergeThread extends Thread {
      ...
      private void doInMemMerge() throws IOException{
        ...
        Path outputPath =
               mapOutputFile.getInputFileForWrite(mapId, spillFileSyncNum.addAndGet(1),mergeOutputSize);
        Writer writer = 
          new Writer(conf, rfs, outputPath,
                     conf.getMapOutputKeyClass(),
                     conf.getMapOutputValueClass(),
      ...
      private int getMapCompletionEvents() throws IOException {
        ...
        MapTaskCompletionEventsUpdate update = 
          umbilical.getMapCompletionEvents(reduceTask.getJobID(), 
                                           fromEventId.get(), 
                                           MAX_EVENTS_TO_FETCH,
                                           reduceTask.getTaskID(), jvmContext,partsFromEventId.get());
        TaskCompletionEvent events[] = update.getMapTaskCompletionEvents();
        NewPartitionAddedEvent npae = update.getNewPartitionAddedEvent(); 
        ...
        //LOG.info("------------in reduce task, start get add new partition"+" npae is null?"+(npae==null));
        if(npae != null && !partitionArrangeFinished.get()){
            Integer[] parts = npae.getAddEdPartitions();
            //LOG.info("---------num new part:"+parts.length);
            //这里可能会重复多次获取同一个cocurrentlist List<MapOutputLocation> loc = mapLocations.get(originalMapOutputLoc.getHost());
            //带来一些效率问题
            for(Map.Entry<TaskAttemptID, MapOutputLocation> en : persistentMapLocations.entrySet()){
            	TaskAttemptID taskId = en.getKey();
            	MapOutputLocation originalMapOutputLoc = en.getValue();
            	List<MapOutputLocation> loc = mapLocations.get(originalMapOutputLoc.getHost());
            	for(int p : parts){
        			MapOutputLocation mapOutput = new MapOutputLocation(originalMapOutputLoc.getTaskAttemptId(),
        					originalMapOutputLoc.getHost(),
        					new URL(
        							originalMapOutputLoc.getOutputLocation().toString()
        							.substring(0, originalMapOutputLoc.getOutputLocation().toString().indexOf("&reduce")) + 
        							"&reduce=" + p));
        			loc.add(mapOutput);
        			numNewMaps++;
        			//LOG.info("--------------------------------added mapoutputlocation "+mapOutput);
        		}
            }
            addNewPartitions(parts); 
            
            partitionArrangeFinished.set(npae.partitionArrangeFinished());
            //LOG.info("!!!!!!!!!!!!!!!!!!!finished="+partitionArrangeFinished.get());
        }
              ...
              synchronized(partitions){//为每一个指定的partition生成对应的MapOutputLocation
            	  for(Integer part:partitions){
            		  URL mapOutputLocation = new URL(event.getTaskTrackerHttp() + 
                              ...
                              "&map=" + taskId + 
                              "&reduce=" + part);
            		  MapOutputLocation maploc = new MapOutputLocation(taskId, host, mapOutputLocation);
                      loc.add(maploc);
                      if(!persistentMapLocations.containsKey(taskId)){
                    	  persistentMapLocations.put(taskId, maploc);
                      }
                      //persistentMapLocation只要保存一份TaskID-host-URL with partitio不要每个partition都保存
