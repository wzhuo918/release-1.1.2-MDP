...
public class JobInProgress {
  ...
  public synchronized Task obtainNewReduceTask(TaskTrackerStatus tts,
    ...
    /**
     * add by xujian 
     * 功能：当map执行到某个特定的进度（100%）的时候，开始进行reduce任务，否则不开启reduce任务
     */
    
//    int times;//分配的次数
//    times = 2;
//    //    List<> list = new List<>();
//    float tmp = (float)finishedMapTasks/(float)numMapTasks;
//    if(tmp<0.5){
//    	return null;
//    }
  ...
  /**
   * Find a non-running task in the passed list of TIPs for a certain tasktracker(added by xujian,2012.9.24)
   ...
   */
  private synchronized TaskInProgress findTaskFromListForCertainReducerTasktracker(
      Collection<TaskInProgress> tips, TaskTrackerStatus ttStatus,
      int numUniqueHosts,
      boolean removeFailedTip) {
	//1.if the tt is shceduled in the first circle
	//0.if the tt is NOT shceduled in the first circle
	 /**
	  * get the partition value for the Host
	  */
	//int partitionvalue;
	//partitionvalue = JobTracker.reducetaskschedule(ttStatus.getHost());
	int flag = 0;
    ...
    while (iter.hasNext()) {
      ...
      if (tip.isRunnable() && !tip.isRunning()) {
        ...
        if (!tip.hasFailedOnMachine(ttStatus.getHost()) || 
          ...
          //add our new ideas.
        	//get the partition value for this tasktracker
           int partition_value_needed = -1;
           //partition_value_needed = partitionvalue;
           int partition_value = tip.idWithinJob();
           if(partition_value == partition_value_needed)
        	{
        	   flag = 1;
        	   iter.remove();
        	   LOG.info("ttStatus.getHost is :" + ttStatus.getHost() + " Partitioner is " + tip.getIdWithinJob() + " manual shceduled");
        	   synchronized(JobTracker.scheduledornot)
        		 {
        		   //JobTracker.scheduledornot[partitionvalue]=-1;
        		 }
        	   return tip;	
        	}
    ...
    //if tt needed to be schedule in the secnd circle ,execuse the following programs
    if(flag == 0)
    {
    	Iterator<TaskInProgress> iter_t = tips.iterator();
        while (iter_t.hasNext()) {
          TaskInProgress tip = iter_t.next();
          ...
          if (tip.isRunnable() && !tip.isRunning()) {
            ...
            if (!tip.hasFailedOnMachine(ttStatus.getHost()) || 
                 tip.getNumberOfFailedMachines() >= numUniqueHosts) {
              // check if the tip has failed on all the nodes
            	LOG.info("ttStatus.getHost is :" + ttStatus.getHost() + " Partitioner is " + tip.getIdWithinJob() + " Random shceduled");
              iter_t.remove();
              return tip;
            } else if (removeFailedTip) { 
              // the case where we want to remove a failed tip from the host cache
