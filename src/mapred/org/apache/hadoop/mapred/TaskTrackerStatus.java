...
public class TaskTrackerStatus implements Writable {
	 ...
	 * @功能 用于判断是否为空
	 */
	String InfoFromTaskTracker = "These is null";
	/**
	...
	static class ResourceStatus implements Writable {
		...
		public void setCumulativeCpuTime(long cumulativeCpuTime) {
			this.cumulativeCpuTime = cumulativeCpuTime > 0 ? cumulativeCpuTime : UNAVAILABLE;
		}
   ...
   */
	...
	/**
   */
	public TaskTrackerStatus(String trackerName, String host, int httpPort, List<TaskStatus> taskReports,
		int taskFailures, int dirFailures, int maxMapTasks, int maxReduceTasks) {
		this.trackerName = trackerName;
		this.host = host;
   ...
   */
	...
	public void write(DataOutput out) throws IOException {
		...
		resStatus.write(out);
		//add by xujian
		//  out.writeLong(id_num);
		//  out.writeUTF(InfoFromTaskTracker);
		Text.writeString(out, InfoFromTaskTracker);
		//end add
	...
	public void readFields(DataInput in) throws IOException {
		...
		this.InfoFromTaskTracker = Text.readString(in);
		taskReports.clear();
